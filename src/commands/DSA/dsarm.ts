import { replyToMessage } from "@/discord/replyToMessage";
...
export const DsaRmCommand = declareCommand({
  ...
  run: Effect.fn("DsaRmCommand.run")(function* (message: Message, args: string[]) {
    ...
    if (!pref) {
      const argsMsg = yield* i18n.t("command.dsarm.args");
      yield* replyToMessage(message, { content: argsMsg });
      return;
    }
    const dsaCharRepo = yield* DsaCharRepository;

    if (!(yield* dsaCharRepo.getCharacter(pref))) {
      const noSuchChar = yield* i18n.t("command.dsarm.noSuchChar", { pref });
      yield* sendMessage(channel, { content: noSuchChar });
      return;
    }
    yield* dsaCharRepo.deleteCharacter(pref);
    const successMsg = yield* i18n.t("command.dsarm.success", { pref });
