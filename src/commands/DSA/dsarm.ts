...
export default class Dsarm extends Command {
  ...
  help = { show: true, usage: `${this.prefix}dsarm <character>` };
  async run(client: Bot, message: Message, args: string[], language: lang) {
    ...
    if (!(await client.db.getDSAChar(pref))) {
      await message.channel.send({ content: language.command.dsarm.noSuchChar.replace("{pref}", pref) });
      return;
    }
    await client.db.deleteDSAChar(pref);
    await message.channel.send({ content: language.command.dsarm.success.replace("{pref}", pref) });
  }
}
