...
export default class Kiss extends GifCommand {
    ...
    async run(client: Bot, message: Message, args: string[], language: lang) {
        const gif: string = await client.db.getgif("kiss", await client.db.getgiftype(message.author));
        ...
        let responseString = "";
        if (userB == "") {
            responseString = (await client.random.choice(language.command.kiss.singleUser)).replace(/{a}/g, userA);
        } else {
            responseString = (await client.random.choice(language.command.kiss.multiUser)).replace(/{a}/g, userA).replace(/{b}/g, userB);
        }
