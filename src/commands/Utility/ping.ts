...
export const PingCommand = declareCommand({
  ...
  run: Effect.fn("PingCommand.run")(function* (message: Message, _args: string[]) {
    ...
    const apiLatency = yield* i18n.t("command.ping.apiLatency", { latency: Math.round(client.ws.ping) });
    const latency = yield* i18n.t("command.ping.latency", { latency: msg.createdTimestamp - message.createdTimestamp });
    ...
    yield* editMessage(msg, { content: "<:check_4:498523284804075541> Pong!", embeds: [embed] });
  }),
});
