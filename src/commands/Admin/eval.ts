...
export default class Eval extends Command {
  ...
  async run(_client: Bot, message: Message, args: string[], language: ILanguage) {
    ...
    if (!super.isOwner(message)) {
      await message.channel.send({ content: language.command.eval.permissionError });
      return;
    }
    ...
    try {
      ...
      if (evaled) {
        const segments = splitMessage("```js\n{evaled}```".replace("{evaled}", inspect(evaled)), {
          append: "```",
          prepend: "```js\n",
          char: "\n",
        });
        await Promise.all(segments.map((m) => (message.channel as SendableChannels).send({ content: m })));
      }
    } catch (err) {
      logger.error(
        `at commands/eval by ${message.author.tag}, using lines ${args.join(" ")}, resulting in ${JSON.stringify(err)}`,
      );
