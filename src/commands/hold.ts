...
export default class Hold extends GifCommand {
    ...
    async run(client: Bot, message: Message, args: string[], language: lang) {
        const gif: string = await client.db.getgif("handhold", await client.db.getgiftype(message.author));
        ...
        let responseString = "";
        if (userB == "") {
            responseString = (await client.random.choice(language.command.hold.singleUser)).replace(/{a}/g, userA);
        } else {
